...
if (!dir.exists("./climateData/pcp/"))
	dir.create("./climateData/pcp/")

if (!dir.exists(paste0("./climateData/pcp/", year)))
{
	download.file(paste0(basurl, info, year, end),
		destfile = zout, method = "wget")

	## Unzip in the folder climateData
	unzip(zout, exdir = "./climateData/pcp/")
	ls_asc = list.files(path = paste0("./climateData/pcp/", year), pattern = ".asc")
	if (length(ls_asc) != 12)
