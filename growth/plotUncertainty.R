...
## Uncertainty file
confInt = fread("./confInt_allSpecies_growth.csv")
...
nbParameters = confInt[, .N]/nbSpecies # Should be 20
nbPlotsPerPage = 4
...
for (i in 1:nbPages)
{
	tikz(paste0("./confIntPlot/page_", i, "_growth.tex"), width = 3.1, height = 3.1)
	for (j in 1:nbPlotsPerPage)
	{
		...
		## Get the 90% interval
		sub_dt[, inf90p := median - qnorm(1-((1-level)/2))*sd]
		sub_dt[, sup90p := median + qnorm(1-((1-level)/2))*sd]

		## Plot
		# Bounds of plot for climVar
		min_bound = min(sub_dt[, inf90p])
		max_bound = max(sub_dt[, sup90p])
		...
		{
			m = unlist(sub_dt[k, inf90p])
			M = unlist(sub_dt[k, sup90p])
...
if (lastPlot != 0)
{
	tikz(paste0("./confIntPlot/page_", nbPages + 1, "_growth.tex"), width = 3.1, height = 3.1)
	for (j in 1:lastPlot)
	{
		## Extract all the species-specific values for params (i - 1)*nbPlotsPerPage + j
		index = seq((i - 1)*nbPlotsPerPage + j, confInt[, .N], by = nbParameters)
