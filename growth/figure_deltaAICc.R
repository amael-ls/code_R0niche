...
pdf("figure_deltaAICc.pdf", height = 8, width = 8)
par(mar = c(5,6,2,2))
plot(ls_delta_aic[, .(x, log10_Delta_AICc)], xlab = "", ylab = "", main = "",
    yaxt = "n", xaxt = "n", yaxs="i", xaxs="i", lwd = 1, xlim = c(m_x, M_x), ylim = c(m_y, M_y),
	type = "p", pch = 19)
lines(unique(ls_delta_aic[, .(x, average)]), type = "l", lwd = 3,
	col = "#5CA7DD")
points(unique(ls_delta_aic[, .(x, average)]), col = "#FFAD33", pch = 19, cex = 1.2)
...
lines(unique(ls_delta_aic[, .(x, average)]), type = "l", lwd = 2,
	col = "#5CA7DD")
...
title(xlab = "Model", ylab = "$ \\log_{10}(\\frac{\\Delta AIC_c}{\\min(\\Delta AIC_c)}) $")
dev.off()

10^1.1564457/10^0.5724304
