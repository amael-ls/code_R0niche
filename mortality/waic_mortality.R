...
my_waic <- function(x, species_id, formula_id, rm_chains = NULL, ...)
{
	...
	if (is.stanjm(x)) {
		...
		if (file.exists(paste0("array_", species_id, "/out", formula_id, ".rds")))
		{
			print(paste0("array_", species_id, "/out", formula_id, ".rds exists. Loading ..."))
			args <- readRDS(paste0("array_", species_id, "/out", formula_id, ".rds"))
		} else {
...
(ls_files = list.files(path = loadPath, pattern = "^model_"))
...
saveRDS(waic_model, paste0(loadPath, "waic", formula_id, ".rds"))
