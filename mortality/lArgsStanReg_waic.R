	...
	reloo_or_kfold = FALSE, species_id, formula_id, rm_chains = NULL, ...)
{
	...
	if (is.mer(object))
	{
		...
		if (unique(z@x == 1))
		{
			...
			Matrix::writeMM(z, paste0("array_", species_id, "/z", formula_id, ".txt"))
			# Position of the 1s (i.e., the only non-zero coeff of the matrix)
			pos_1 = data.table::fread(paste0("array_", species_id, "/z", formula_id, ".txt"), header = FALSE)
			n_row = nrow(z)
			n_col = ncol(z)
			z2 = matrix(data = 0, nrow = n_row, ncol = n_col)
			z2_ind = (pos_1[, V2] - 1)*n_row + pos_1[, V1]
			z2[z2_ind] = 1
			test = z2 - z # Note that test is a sparse matrix
			if (unique(test@x) == 0)
	...
	saveRDS(out, paste0("array_", species_id, "/out", formula_id, ".rds"))
	print("lArg done")
	return(out)
}
