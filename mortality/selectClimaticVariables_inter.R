...
if (formula_id == 1)
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		(dbh + I(dbh^2))*(annual_mean_temperature + I(annual_mean_temperature^2) +
			annual_precipitation + I(annual_precipitation^2)),
		data = mortality_data_norm,
		family = binomial(link = "cloglog"), offset = log(deltaYear), iter = 8000, cores = 4, chains = 4,
		prior = normal(),
		...
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_1_inter.rds"))
}
...
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		(dbh + I(dbh^2))*(mean_temperature_of_driest_quarter +
		...
		# diagnostic_file = paste0(savePath, "diagnostic", formula_id, ".csv"),
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_2_inter.rds"))
}
...
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		(dbh + I(dbh^2))*(mean_temperature_of_warmest_quarter +
		...
		# diagnostic_file = paste0(savePath, "diagnostic", formula_id, ".csv"),
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_3_inter.rds"))
}
...
if (formula_id == 4)
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		(dbh + I(dbh^2))*(mean_temperature_of_driest_quarter +
			I(mean_temperature_of_driest_quarter^2) +
			precipitation_of_driest_month + I(precipitation_of_driest_month^2)),
		data = mortality_data_norm,
		family = binomial(link = "cloglog"), offset = log(deltaYear), iter = 8000, cores = 4, chains = 4,
		prior = normal(),
		prior_intercept = normal(),
		...
		# diagnostic_file = paste0(savePath, "diagnostic", formula_id, ".csv"),
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_4_inter.rds"))
}
...
if (formula_id == 5)
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		...
		# diagnostic_file = paste0(savePath, "diagnostic", formula_id, ".csv"),
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_5_inter.rds"))
}
...
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		(dbh + I(dbh^2))*(min_temperature_of_coldest_month +
		...
		# diagnostic_file = paste0(savePath, "diagnostic", formula_id, ".csv"),
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_6_inter.rds"))
}
...
if (formula_id == 7)
{
	model = stan_glm(formula = deltaState ~ 1 + canopy_status +
		...
		# diagnostic_file = paste0(savePath, "diagnostic", formula_id, ".csv"),
		algorithm = "sampling")
		saveRDS(model, paste0(savePath, "model_7_inter.rds"))
}
